...
from '../todo.service';
@Component({ selector: 'app-tododescription', templateUrl: './tododescription.component.html', styleUrls: ['./tododescription.component.css'] }) export class TododescriptionComponent implements OnInit {
       ...
       if(this.toggle == "on") {
          ...
          for(let i = 0; i < gettodos.length; i++) {
              if (gettodos[i].label == Todoname) {
                  let p = {
                      label: gettodos[i].label,
                      date: gettodos[i].date,
                      time: gettodos[i].time,
                      Description: gettodos[i].Description
                  }
